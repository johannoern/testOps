...
# config
# STACK_NAME=powertestingStack
INPUT=$(cat ./execution_input.json)  # or use a static string
...
do
    ...
    if test "$STATUS" == "RUNNING"; then
        ...
        # retrieve output
        aws stepfunctions describe-execution --execution-arn $EXECUTION_ARN --query 'output' --output text > "$OUTPUT"
        break
    fi
done
