...
def get_smallest_memory(self, terraform_dir, function_name, invoker, payload, provider_state, provider_vars, mem_attribute):
    ...
    time, response = deployer_baas.invoke_function(function_name, self._region, invoker, min_mem, payload)
...
def get_biggest_memory(self, terraform_dir, function_name, invoker, min_speedup, payload, provider_state, provider_vars, mem_attribute):
    tf_state = terraform.get_tfstate(terraform_dir)
    deployed_mem = deployer_baas.deployed_mem(tf_state, provider_state, mem_attribute)
    max_mem = min(deployed_mem)
    current_time, response = deployer_baas.invoke_function(function_name, self._region, invoker, max_mem, payload)
